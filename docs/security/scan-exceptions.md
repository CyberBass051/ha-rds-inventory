...
| `CKV_AWS_115` | `aws_lambda_function.bootstrap`, `aws_lambda_function.write` | Account-level Lambda concurrency limit is too low to reserve capacity for either function without breaching AWS's mandatory 10-unit unreserved floor. Left unreserved; acceptable at this project's traffic scale (one-time invocation, 5-10 req/s test load). | Permanent for this project | Revisit if a limit increase is requested or this account's usage grows. |
| `CKV2_AWS_57` | `aws_secretsmanager_secret.app_user` | The stored password is a bookkeeping requirement of RDS Proxy (see ADR 0005) — it is not the actual authentication factor at connection time, since `iam_auth = "REQUIRED"` mandates a short-lived IAM token for every connection regardless of the stored password's age. Rotation infrastructure would protect a credential that isn't load-bearing. | Permanent for this project | Revisit if this project's design ever drops IAM auth in favor of direct password authentication through the Proxy. |
